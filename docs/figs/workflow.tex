...
\begin{tikzpicture}[diagram]
  ...
  \node (workspace) [archive] {{\bf Workspace} \small{Events, Stations, Waveforms, Auxiliary
    Data}};
  ...
  % Main connections
  \draw[arrow-flow] (collector) to [bend left] (station-metrics);
  \draw[arrow-flow] (station-metrics) to [bend left] (generate-results);
  \draw[arrow-flow] (collector) to [bend right] (process-waveforms);
  \draw[arrow-flow] (process-waveforms) to [bend right=15] (waveform-metrics);
  \draw[arrow-flow] (waveform-metrics) to [bend right] (generate-results);
